...
usage() {
  cat <<'EOF'
Download scene-reconstruction weights into egosim-opensource/artifacts/models/.

Usage:
  bash continuous_simulation/scripts/download_scene_weights.sh [options]

Options:
  --with-qwen               Also download Qwen3-VL-4B-Instruct (phrase prediction)
...
if [[ "${WITH_QWEN}" == "1" ]]; then
  echo "Set models.qwen_vl_root: ../artifacts/models/qwen-vl/Qwen3-VL-4B-Instruct in configs/project.yaml"
fi
if [[ "${WITH_AOT}" != "1" ]]; then
